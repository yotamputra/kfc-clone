...
describe("PUT /cuisines/:id", () => {
  describe("Success update cuisine", () => {
    test("Success 200 OK", async () => {
      const res = await request(app)
        .put("/cuisines/2")
        .send({
          name: "Fried Chicken",
          description: "Delicious deep fried chicken.",
          price: 30000,
          imgUrl:
            "https://i0.wp.com/www.savourous.com/wp-content/uploads/2019/05/KFC-chicken1.jpg?fit=3456%2C3456&ssl=1",
          categoryId: 1,
        })
        .set({
      ...
      // console.log(res.body, "<--- update cuisine");
      expect(res.status).toBe(200);
      expect(res.body[0]).toHaveProperty("id", expect.any(Number));
      expect(res.body[0]).toHaveProperty("name", "Fried Chicken");
      expect(res.body[0]).toHaveProperty(
        "description",
        "Delicious deep fried chicken."
      );
      expect(res.body[0]).toHaveProperty("price", 30000);
      expect(res.body[0]).toHaveProperty(
        "imgUrl",
        "https://i0.wp.com/www.savourous.com/wp-content/uploads/2019/05/KFC-chicken1.jpg?fit=3456%2C3456&ssl=1"
      );
      expect(res.body[0]).toHaveProperty("categoryId", 1);
      expect(res.body[0]).toHaveProperty("authorId", 1);
    });
  });

  describe("Failed update cuisine with no token", () => {
    test("Failed 401 no access token", async () => {
      const res = await request(app).put("/cuisines/2").send({
        name: "Fried Chicken",
        description: "Delicious deep fried chicken.",
        price: 30000,
        imgUrl:
  ...
  describe("Failed update cuisine invalid token", () => {
  ...
  describe("Failed update cuisine, missing data", () => {
  ...
  describe("Failed update cuisine, authorization denied", () => {
  ...
  describe("Failed update cuisine invalid req.body", () => {
